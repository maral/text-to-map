let romanNumerals;
const singleRomanNumeralPattern = /X{1,3}(IX|IV|V?I{0,3})|(IX|IV|VI{0,3})|(I{1,3})/g;
const romanNumeralsListPattern = /(X{1,3}(IX|IV|V?I{0,3})|(IX|IV|VI{0,3})|(I{1,3}))(, ?(X{1,3}(IX|IV|V?I{0,3})|(IX|IV|VI{0,3})|(I{1,3})))+$/;
const romanNumeralsRangePattern = /([IVXLCDM]+) ?- ?([IVXLCDM]+)$/;
export const splitStreetViaRomanNumerals = (input) => {
    const list = romanNumeralsListPattern.exec(input);
    if (list) {
        const streetName = input.substring(0, input.indexOf(list[0])).trim();
        return extractRomanNumerals(list[0]).map((numeral) => `${streetName} ${numeral}`);
    }
    else {
        ...
        if (range) {
            ...
            return romanNumerals
                .slice(romanNumerals.indexOf(bounds[0]), romanNumerals.indexOf(bounds[1]) + 1)
                .map((numeral) => `${streetName} ${numeral}`);
        }
    }
    return [input];
};
const extractRomanNumerals = (input) => {
    let n;
    const numerals = [];
    do {
